...
describe('Data Sync Validation', () => {
  ...
  describe('email-service Integration Sync', () => {
    test('should validate email-service authentication before sync', async () => {
      ...
      if (health.status === 'unhealthy') {
        console.warn('⚠️  email-service is unhealthy, sync tests may fail:', health.details);
      }
    ...
    test('should sync user data from email-service', async () => {
      const result = await syncService.validateEntitySync('email-service', 'user');
      ...
      if (!result.success) {
        console.warn('⚠️  user sync from email-service failed:');
    ...
    test('should sync project data from email-service', async () => {
      const result = await syncService.validateEntitySync('email-service', 'project');
      ...
      if (!result.success) {
        console.warn('⚠️  project sync from email-service failed:');
        result.errors.forEach(error => console.warn(`   - ${error}`));
      } else {
        expect(result.recordsStored).toBeGreaterThanOrEqual(0);

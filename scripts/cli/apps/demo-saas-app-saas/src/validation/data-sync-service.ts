...
export class DataSyncService {
  ...
  async validateAllIntegrationSync(): Promise<SyncReport> {
    ...
    // Validate email-service sync
    try {
      const emailServiceResults = await this.validateIntegrationSync('email-service');
      results.push(...emailServiceResults);
    } catch (error) {
      console.error('❌ email-service sync validation failed:', error);
      results.push({

...
export default function ReservationsDetailPage() {
  ...
  useEffect(() => {
    if (params.id) {
      fetch(`/api/Reservation/${params.id}`)
        .then(res => res.json())
        .then(result => {
          setData(result)
          setLoading(false)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-3xl font-bold">Reservations Details</h1>
        </div>
      ...
      <Card className="p-6">
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <dt className="font-semibold text-gray-700">propertyId</dt>
            <dd className="mt-1 text-gray-900">
              {data.propertyId || '-'}
            </dd>
          </div>
          <div>
            <dt className="font-semibold text-gray-700">guestId</dt>
            <dd className="mt-1 text-gray-900">
              {data.guestId || '-'}
            </dd>
          </div>
          <div>
            <dt className="font-semibold text-gray-700">checkIn</dt>
            <dd className="mt-1 text-gray-900">
              {data.checkIn || '-'}
            </dd>
          </div>
          <div>
            <dt className="font-semibold text-gray-700">checkOut</dt>
            <dd className="mt-1 text-gray-900">
              {data.checkOut || '-'}
            </dd>
          </div>
          <div>
            <dt className="font-semibold text-gray-700">totalPrice</dt>
            <dd className="mt-1 text-gray-900">
              {data.totalPrice || '-'}
            </dd>
          </div>

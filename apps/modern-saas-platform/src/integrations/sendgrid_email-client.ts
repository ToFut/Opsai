...
export interface SendgridEmailConfig extends BaseIntegrationConfig {
  apiKey?: string;
  rateLimitRpm?: number;
}

export class SendgridEmailClient extends BaseIntegrationClient {
  private integrationConfig: SendgridEmailConfig;

  constructor(config: SendgridEmailConfig) {
    // Validate configuration
    SendgridEmailClient.validateConfig(config);
    
    super(config, {
      api_key: config.apiKey || process.env.SENDGRID_EMAIL_API_KEY
    });
    
    this.integrationConfig = config;
  }

  private static validateConfig(config: SendgridEmailConfig): void {
    if (!config.baseUrl) {
      throw new ConfigurationError('baseUrl is required');
    }
    
    
    if (!config.apiKey && !process.env.SENDGRID_EMAIL_API_KEY) {
      throw new ConfigurationError('api_key is required. Set config.apiKey or environment variable SENDGRID_EMAIL_API_KEY');
    }
  }
...
export function createSendgridEmailClient(): SendgridEmailClient {
  const envPrefix = 'SENDGRID_EMAIL';
  ...
  const config: SendgridEmailConfig = {
    ...
    rateLimitRpm: parseInt(process.env.SENDGRID_EMAIL_RATE_LIMIT_RPM || '600'),
...
export const sendgridEmailClient = createSendgridEmailClient();
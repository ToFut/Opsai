...
export class DataSyncService {
  ...
  async validateAllIntegrationSync(): Promise<SyncReport> {
    ...
    // Validate stripe_payments sync
    try {
      const stripePaymentsResults = await this.validateIntegrationSync('stripe_payments');
      results.push(...stripePaymentsResults);
    } catch (error) {
      console.error('❌ stripe_payments sync validation failed:', error);
      results.push({
    ...
    // Validate sendgrid_email sync
    try {
      const sendgridEmailResults = await this.validateIntegrationSync('sendgrid_email');
      results.push(...sendgridEmailResults);
    } catch (error) {
      console.error('❌ sendgrid_email sync validation failed:', error);
      results.push({
    ...
    // Validate slack_notifications sync
    try {
      const slackNotificationsResults = await this.validateIntegrationSync('slack_notifications');
      results.push(...slackNotificationsResults);
    } catch (error) {
      console.error('❌ slack_notifications sync validation failed:', error);
      results.push({
    ...
    // Validate analytics_db sync
    try {
      const analyticsDbResults = await this.validateIntegrationSync('analytics_db');
      results.push(...analyticsDbResults);
    } catch (error) {
      console.error('❌ analytics_db sync validation failed:', error);
      results.push({
  ...
  private async validateIntegrationSync(integrationName: string): Promise<SyncResult[]> {
    ...
    try {
      const organizationResult = await this.validateEntitySync(integrationName, 'Organization');
    ...
    // Test Task sync
    try {
      const taskResult = await this.validateEntitySync(integrationName, 'Task');

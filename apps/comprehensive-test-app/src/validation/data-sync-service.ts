...
export class DataSyncService {
  ...
  async validateAllIntegrationSync(): Promise<SyncReport> {
    ...
    // Validate email_service sync
    try {
      const emailServiceResults = await this.validateIntegrationSync('email_service');
      results.push(...emailServiceResults);
    } catch (error) {
      console.error('❌ email_service sync validation failed:', error);
      results.push({

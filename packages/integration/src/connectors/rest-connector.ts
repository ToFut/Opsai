...
import { ConnectorConfig, RESTEndpoint, RESTAuthentication } from '../types';
...
export class RESTConnector extends BaseConnector {
  ...
  async initialize(): Promise<void> {
    ...
    if (!isConnected) {
      throw new IntegrationError('Failed to initialize REST connector');
    }
  }
  ...
  async executeRequest(
    ...
    if (!this.checkRateLimit(endpoint)) {
      throw new IntegrationError('Rate limit exceeded', 'RATE_LIMIT_EXCEEDED');
    }
    ...
    try {
      ...
      throw new IntegrationError('Request failed', 'REQUEST_FAILED', error);
    }
  }

  async executeEndpoint(endpointConfig: RESTEndpoint, data?: any): Promise<any> {
  ...
  private async refreshOAuth2Token(): Promise<void> {
    if (!this.authentication || this.authentication.type !== 'oauth2') return;

    try {
      const tokenResponse = await axios.post(this.authentication.tokenUrl!, {
      ...
      throw new IntegrationError('Failed to refresh OAuth2 token', 'AUTH_FAILED', error);
    }
  }

...
exports.APIMarketplace = exports.API_MARKETPLACE = void 0;
exports.API_MARKETPLACE = {
...
class APIMarketplace {
    ...
    static getAllConnectors() {
        return Object.values(exports.API_MARKETPLACE);
    }
    // Get connectors by category
    static getByCategory(category) {
        return Object.values(exports.API_MARKETPLACE).filter(api => api.category === category);
    }
    // Search connectors
    static search(query) {
        const lowerQuery = query.toLowerCase();
        return Object.values(exports.API_MARKETPLACE).filter(api => api.name.toLowerCase().includes(lowerQuery) ||
    ...
    static getQuickSetupConnectors() {
        return Object.values(exports.API_MARKETPLACE).filter(api => api.quickSetup);
    }
    // Generate integration config from template
    static generateIntegrationConfig(connectorId) {
        const template = exports.API_MARKETPLACE[connectorId];
        if (!template) {
    ...
    static generateDefaultEndpoints(template) {
        const endpoints = [];
    ...
    static generateDefaultTransformations(template) {
    ...
    static async oneClickSetup(connectorId, credentials) {
        try {
            const template = exports.API_MARKETPLACE[connectorId];
            if (!template) {
                throw new Error(`Connector not found: ${connectorId}`);
